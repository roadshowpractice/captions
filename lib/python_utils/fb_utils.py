# ==================================================
# fb_utils.py - Utility functions for video processing
# ==================================================
...
# Function List:
#
# - check_cookie_freshness(cookie_path: str, max_age_days: int = 10) -> bool:
#    Checks if a cookie file is older than the allowed maximum age.
#
...
#
# - extract_fb_comments_selenium(url: str, cookies_path: str, wait_time: int = 5) -> list:
#    Uses Selenium to extract Facebook comments from a video page.
#
...
def check_cookie_freshness(cookie_path: str, max_age_days: int = 10) -> bool:
    """
    Checks if a cookie file is older than the allowed maximum age.

    Args:
        cookie_path (str): Path to the cookie file.
        max_age_days (int): Maximum allowed age in days.
    ...
    if not os.path.exists(cookie_path):
        return False
    file_age = (time.time() - os.path.getmtime(cookie_path)) / 86400  # in days
    if file_age > max_age_days:
        logger.warning(f"⚠️ Cookie file is {int(file_age)} days old. Consider refreshing.")
        return False
    return True
...
def extract_fb_comments_selenium(url: str, cookies_path: str, wait_time: int = 5) -> list:
    """
    Uses Selenium to extract Facebook comments from a video page.

    Args:
        url (str): Facebook video URL.
        cookies_path (str): Path to the cookies.txt file (Netscape format).
        wait_time (int): Time to wait for page load and scrolling.

    Returns:
        list: Extracted comment strings.
    """
    logger = logging.getLogger(__name__)
    logger.info(f"🚀 Launching Selenium to extract comments from: {url}")
    logger.info(f"🔑 Using cookie file: {cookies_path}")

    chrome_options = Options()
    # REMOVE HEADLESS for now if debugging:
    ...
    try:
            ...
            for line in f:
                ...
                if len(parts) == 7:
                    ...
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to load cookie: {cookie['name']}")

        logger.info("🔁 Re-loading target video URL with session cookies...")
        driver.get(url)
        ...
        buttons = driver.find_elements(By.XPATH, "//div[contains(text(),'View more comments')]")
        for btn in buttons:
            try:
                btn.click()
                time.sleep(1)
            except Exception as e:
                logger.debug("⚠️ Couldn't click 'View more comments'")

        comment_elements = driver.find_elements(By.XPATH, "//div[@data-ad-preview='message']")
        comments = [c.text for c in comment_elements if c.text.strip()]
        logger.info(f"✅ Extracted {len(comments)} comments via Selenium")

        return comments

    except Exception as e:
        logger.error(f"❌ Selenium comment extraction failed: {e}")
        return []
    finally:
        driver.quit()
